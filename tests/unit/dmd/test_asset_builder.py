...
def test_build_asset_name_is_prefixed_with_tfmdfp():
    ...
    result = build_asset(asset_data, [], _make_template(), config)
...
def test_build_asset_uses_xv_hash_code_as_code_reference():
    ...
    config = _make_ingestion_config()
    result = build_asset(asset_data, characteristics, _make_template(), config)
...
def test_build_asset_sets_correct_template_id_and_code():
    asset_data = _make_asset_data()
    config = _make_ingestion_config()
    result = build_asset(asset_data, [], _make_template(template_id=99, code="VALVE"), config)
...
def test_build_asset_sets_main_hierarchy_parent():
    asset_data = _make_asset_data()
    config = _make_ingestion_config(main_hierarchy_parent=7)
    result = build_asset(asset_data, [], _make_template(), config)
