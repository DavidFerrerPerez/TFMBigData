...
def main():
    ...
    extraction_config = load_ingestion_config("config/ingestion.example.yaml")
    ...
    for table in extraction_config.tables:

        logging.info(f"Reading table '{table}' from schema '{extraction_config.source.schema_name}'...")

        try:
            df = read_table_from_postgres(
                extraction_config.source.schema_name,
                table,
                spark
            )
